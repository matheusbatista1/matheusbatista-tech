...
export async function POST(request: Request) {
  ...
  const [perMinute, perDay] = await Promise.all([
    container.ai.rateLimits.chat.limit(`ai-project-description:${ip}`),
    container.ai.rateLimits.daily.limit(`ai-project-description:${ip}`),
  ]);
  ...
  try {
    const result = await container.useCases.generateProjectDescription.execute({
    ...
    console.error("ai/project-description route error", error);
